...
import { AFFILIATE_TERMS_MD } from '@/content/affiliate-legal';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: localeAlternates('/affiliate/terms', locale),
  };
}

// Rendered verbatim in English in every locale — legal wording is not
// translated. See src/content/affiliate-legal.ts.
export default async function AffiliateTermsPage({ params }: Props) {
  const { locale } = await params;
    ...
    <div className="bg-paper-alt text-ink">
      ...
      <main className="mx-auto w-full max-w-[760px] px-6 py-14 md:py-20">
        <article className="min-w-0">
          <MarkdownBody>{AFFILIATE_TERMS_MD}</MarkdownBody>
        </article>
      </main>
      <SiteFooter />
    </div>
